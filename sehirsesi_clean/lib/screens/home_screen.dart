...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  // Varsayılan değerler — SharedPreferences yüklenince üzerine yazılır
  ...
  Future<void> _loadSavedLocation() async {
    ...
    // Şehir seçim ekranından kaydedilen il
    final savedCity = prefs.getString('selected_city');
    // Konum seçiciden kaydedilen ilçe
    ...
    if (savedCity != null && savedCity.isNotEmpty) {
      ...
      // Kayıtlı ilçe varsa ve o ile ait ilçeler arasındaysa kullan
      final availableDistricts = TurkeyData.districts[province] ?? [];
      if (savedDistrict != null &&
          savedDistrict.isNotEmpty &&
          availableDistricts.contains(savedDistrict)) {
        district = savedDistrict;
      } else {
        // Yoksa ilin ilk ilçesini varsayılan yap
        district = availableDistricts.isNotEmpty ? availableDistricts.first : '';
      }
    } else {
      // Hiç şehir seçilmemişse ilk ile git
      province = TurkeyData.provinces.first;
      district = TurkeyData.districts[province]?.first ?? '';
    }
    ...
    return AppBar(
      ...
      title: Column(
        children: [
          ...
          // Sadece her ikisi de doluysa göster
          ...
          else if (_selectedProvince.isNotEmpty)
            Text(
              _selectedProvince,
              style: TextStyle(
                fontSize: 12,
  ...
  Widget _buildBody() {
    ...
    switch (_selectedTab) {
      ...
      case 2:
        return MunicipalityDashboardScreen(province: _selectedProvince);
      case 3:
  ...
  Widget _buildAssistantTab() {
    ...
    return _AIAssistantView(
        neighborhood: neighborhood, district: _selectedDistrict);
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _profileRow(Icons.person_outline, 'Kullanıcı Adı', profile?['username'] ?? '-'),
          _profileRow(Icons.email_outlined, 'Email', email.isNotEmpty ? email : '-'),
          ...
          _actionButton(
            ...
            onTap: () => Navigator.push(context,
              MaterialPageRoute(builder: (_) => CitySelectionScreen(
                isFirstTime: false,
                onSelected: (p, d) { setState(() { _selectedProvince = p; _selectedDistrict = d; }); },
              ))),
          ),
  ...
  void _showChangeUsername() {
    final ctrl = TextEditingController(
      text: context.read<AuthService>().profile?['username'] ?? '',
    );
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('Kullanıcı Adını Değiştir', style: TextStyle(fontWeight: FontWeight.w700)),
        content: TextField(
          controller: ctrl,
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final result = await context.read<AuthService>().updateUsername(ctrl.text.trim());
              if (!mounted) return;
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(
                  result['success'] == true
        ...
        SliverPadding(
          ...
          sliver: SliverGrid(
            ...
            delegate: SliverChildBuilderDelegate(
              (context, i) =>
                  _buildNeighborhoodCard(_neighborhoodStats[i]),
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () => Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => FeedbackScreen(province: _selectedProvince, district: _selectedDistrict, neighborhood: _defaultNeighborhoodForFeedback)),
              ),
      ...
      child: Row(
        children: [
          ...
          Column(
            children: [
              _buildMiniStat(
                  '${_neighborhoodStats.length}', 'Mahalle'),
              const SizedBox(height: 8),
              _buildMiniStat(
                '${_neighborhoodStats.fold(0, (s, n) => s + n.totalFeedbacks)}',
                'Geri Bildirim',
        ...
        child: SizedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              _navItem(1, Icons.chat_bubble_outline,
                  Icons.chat_bubble, 'Asistan'),
              const SizedBox(width: 40),
              _navItem(
                  2, Icons.bar_chart_outlined, Icons.bar_chart, 'Raporlar'),
  ...
  String get _defaultNeighborhoodForFeedback {
    final fromProfile = context.read<AuthService>().profile?['neighborhood']?.toString() ?? '';
    if (fromProfile.isNotEmpty) return fromProfile;
    if (_neighborhoodStats.isNotEmpty) return _neighborhoodStats.first.neighborhood;
    final fallback = TurkeyData.neighborhoods[_selectedDistrict];
    if (fallback != null && fallback.isNotEmpty) return fallback.first;
    return '';
  }

  void _openFeedbackScreen() {
    final neighborhood = _defaultNeighborhoodForFeedback;
    if (neighborhood.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Önce konum seçimi tamamlanmalı')),
      );
      return;
    }
    Navigator.push(
  ...
  void _showLocationPicker() {
    showModalBottomSheet(
      ...
      builder: (_) => _LocationPickerSheet(
        ...
        onSelected: (province, district) async {
          // Seçimi kaydet
          final prefs = await SharedPreferences.getInstance();
          await prefs.setString('selected_city', province);
...
class _LocationPickerSheetState extends State<_LocationPickerSheet> {
  ...
  void initState() {
    super.initState();
    _province = widget.selectedProvince.isNotEmpty
        ? widget.selectedProvince
        : TurkeyData.provinces.first;

    final availableDistricts = TurkeyData.districts[_province] ?? [];

    // Mevcut ilçe bu ile aitse kullan, yoksa ilk ilçeyi seç
    if (availableDistricts.contains(widget.selectedDistrict)) {
      _district = widget.selectedDistrict;
    } else {
      _district = availableDistricts.isNotEmpty
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            ...
            onChanged: (v) {
              if (v == null) return;
              final newDistricts = TurkeyData.districts[v] ?? [];
              setState(() {
                _province = v;
                // Province değişince ilçeyi sıfırla — eski ilçeyi taşıma
                _district = newDistricts.isNotEmpty
                    ? newDistricts.first
                    : '';
              });
            },
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            const Text(
              'Mahalleniz hakkında ne öğrenmek istersiniz?',
              textAlign: TextAlign.center,
              style: TextStyle(
